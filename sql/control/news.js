...
exports.detail = async (ctx, next) => {
  ...
  await new Promise((resolve, reject) => {
    News.findById(params.id, function (err, data) {
      if (err) {
        reject(err)
      } else {
        resolve(data)
...
exports.add = async (ctx, next) => {
  ...
  console.log(reqBody)
  await new Promise((resolve, reject) => {
    News.create(Object.assign({}, { created: new Date().getTime() }, reqBody), function (err) {
      if (err) {
        reject('写入错误')
      } else {
        resolve('添加成功')
...
exports.remove = async (ctx, next) => {
  ...
  await new Promise((resolve, reject) => {
    News.remove({ _id: reqBody.id }, function (err) { // 删除
      if (err) {
        reject(err)
      } else {
        resolve('删除成功')
